//再封装 by-LC
//吸附提示背景色
var layerTipColor = '#DC143C';

//吸附提示
function alertAbsorb(msg, id) {
    layer.tips(msg, "#" + id, { tips: [3, layerTipColor] });
}


function alertS(msg, callback) {
    layer.alert(msg, {
        title: "提示", icon: 1, yes: function (index) {
            if (callback)
                callback();
            layer.close(index);
        }
...
function alertF(msg, callback) {
    layer.alert(msg, {
        title: "提示", icon: 2, yes: function (index) {
            if (callback)
                callback();
            layer.close(index);
        }
...
function alertW(msg, callback) {
    layer.alert(msg, {
        title: "提示", icon: 0, yes: function (index) {
            if (callback)
                callback();
            layer.close(index);
        }
